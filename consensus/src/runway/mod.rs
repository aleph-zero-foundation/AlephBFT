use crate::{
    ...
    dissemination::{Addressed, DisseminationMessage, DisseminationRequest, Responder},
    extension::Ordering,
    ...
    MK: MultiKeychain,
{
    ...
    resolved_requests: Sender<DisseminationRequest<FH::Hasher>>,
...
struct RunwayConfig<UFH: UnitFinalizationHandler, MK: MultiKeychain> {
    ...
    resolved_requests: Sender<DisseminationRequest<UFH::Hasher>>,
    ...
    MK: MultiKeychain,
{
    ...
    fn resolve_missing_coord(&mut self, coord: &UnitCoord) {
        if self.missing_coords.remove(coord) {
            self.send_resolved_request_notification(ReconstructionRequest::Coord(*coord).into());
        }
    }
    ...
    fn resolve_missing_parents(&mut self, u_hash: &<UFH::Hasher as Hasher>::Hash) {
        if self.missing_parents.remove(u_hash) {
            self.send_resolved_request_notification(
                ReconstructionRequest::ParentsOf(*u_hash).into(),
            );
        }
    }
    ...
    fn on_reconstruction_request(&mut self, request: ReconstructionRequest<UFH::Hasher>) {
        ...
        if perform_request {
            self.send_message_for_network(RunwayNotificationOut::Request(request.into()));
        }
    }
    ...
    fn send_resolved_request_notification(
        &mut self,
        notification: DisseminationRequest<UFH::Hasher>,
    ) {
...
pub(crate) struct NetworkIO<H: Hasher, D: Data, MK: MultiKeychain> {
    ...
    pub(crate) resolved_requests: Sender<DisseminationRequest<H>>,
}
...
fn initial_unit_collection<'a, H: Hasher, D: Data, MK: MultiKeychain>(
    ...
    let notification =
        RunwayNotificationOut::Request(DisseminationRequest::NewestUnit(keychain.index(), salt));
