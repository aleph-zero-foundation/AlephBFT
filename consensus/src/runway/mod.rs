use crate::{
    ...
    dissemination::{Addressed, DisseminationMessage, Responder},
    extension::Ordering,
...
pub(crate) enum RunwayNotificationOut<H: Hasher, D: Data, S: Signature> {
    ...
    Request(ReconstructionRequest<H>),
    /// An addressed message that should just be send over the network.
    /// TODO(A0-4382): This is the only part of this type that should eventually exist.
    ...
    MK: MultiKeychain,
{
    ...
    messages_from_collection: Receiver<AddressedDisseminationMessage<FH, MK>>,
    resolved_requests: Sender<ReconstructionRequest<FH::Hasher>>,
...
struct RunwayConfig<UFH: UnitFinalizationHandler, MK: MultiKeychain> {
    ...
    messages_from_collection: Receiver<AddressedDisseminationMessage<UFH, MK>>,
    parents_for_creator: Sender<DagUnit<UFH::Hasher, UFH::Data, MK>>,
    resolved_requests: Sender<ReconstructionRequest<UFH::Hasher>>,
    ...
    MK: MultiKeychain,
{
        ...
        match message {
            ...
            Request(node_id, request) => {
                match self.responder.handle_request(request, &self.store) {
                    Ok(response) => self.send_message_for_network(RunwayNotificationOut::Message(
            ...
            NewestUnitRequest(node_id, salt) => {
                ...
                self.send_message_for_network(RunwayNotificationOut::Message(
                    Addressed::addressed_to(response, node_id).into(),
                ))
            }
    ...
    fn resolve_missing_coord(&mut self, coord: &UnitCoord) {
        if self.missing_coords.remove(coord) {
            self.send_resolved_request_notification(ReconstructionRequest::Coord(*coord));
        }
    }
    ...
    fn resolve_missing_parents(&mut self, u_hash: &<UFH::Hasher as Hasher>::Hash) {
        if self.missing_parents.remove(u_hash) {
            self.send_resolved_request_notification(ReconstructionRequest::ParentsOf(*u_hash));
        }
    }

    fn on_reconstruction_request(&mut self, request: ReconstructionRequest<UFH::Hasher>) {
        use ReconstructionRequest::*;
        let perform_request = match &request {
            Coord(coord) => {
                trace!(target: "AlephBFT-runway", "{:?} Dealing with missing coord notification {:?}.", self.index(), coord);
                match self.store.canonical_unit(*coord) {
                    Some(_) => false,
                    None => self.missing_coords.insert(*coord),
                }
            }
            ParentsOf(h) => {
                trace!(target: "AlephBFT-runway", "{:?} Dealing with wrong control hash notification {:?}.", self.index(), h);
                self.missing_parents.insert(*h)
            }
        };
        if perform_request {
            self.send_message_for_network(RunwayNotificationOut::Request(request));
        }
    }
    ...
    fn send_resolved_request_notification(
        &mut self,
        notification: ReconstructionRequest<UFH::Hasher>,
    ) {
    ...
    async fn run(
        ...
        loop {
            futures::select! {
                ...
                message = self.messages_from_collection.next() => {
                    match message {
                        Some(message) => self.send_message_for_network(RunwayNotificationOut::Message(message)),
                        None => debug!("Messages from collection ended."),
                    }
                },
...
pub(crate) struct NetworkIO<H: Hasher, D: Data, MK: MultiKeychain> {
    ...
    pub(crate) resolved_requests: Sender<ReconstructionRequest<H>>,
}
    ...
    SH: SpawnHandle,
{
    ...
    let (messages_for_us, messages_from_collection) = mpsc::unbounded();
    #[cfg(feature = "initial_unit_collection")]
